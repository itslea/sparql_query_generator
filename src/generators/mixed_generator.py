...
class MixedGenerator:
    ...
    def generate_query(self, queries, triples, operator_prob, var_prob):
        ...
        while len(all_queries) < queries:
            ...
            try_counter += 1
            query = ""
            endpoint_data = dh.DataHandler(self.url).fetch_data_mixed(triples)
            endpoint_data_first = endpoint_data['first']
            endpoint_data_second = endpoint_data['second']
            endpoint_data = endpoint_data_first['patterns'] + endpoint_data_second['patterns']
            if len(endpoint_data) >= triples:
                connection = endpoint_data['connection']
